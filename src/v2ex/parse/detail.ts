...
export async function parseDetail(html: string) {
  ...
  const readCount = content.replace(userLink.text(), '').replace(/(.*?)(\d+)(\s次点击)/, '$2').trim();
  const user = {
    name: userLink.text(),
    url: userLink.attr('href'),
  }
  const time = content.replace(new RegExp(`${readCount}\\s次点击`), '').replace(userLink.text(), '').replace(/·/g, '').trim();
