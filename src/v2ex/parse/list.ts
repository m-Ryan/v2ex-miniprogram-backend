  ...
  const listData =$('#Main .box .item tbody').toArray().map((child)=> {
    ...
    return {
      ...
      last_replay: {
       ...
       user_name_url: replayer && replayer.attr('href'),
      },
      replay_count: currentNode.find('.count_livid') ? currentNode.find('.count_livid').text() : 0
    }
   })
